...
int main (int argc, char* argv[])
{
	...
	main_box.setTitle("Bot Evaluation Machine");
	...
	t_desc += ", quit: ";
	t_desc << tui::COLOR::GREEN << tui::input::getKeyName('q');
		...
		case 2:
            if (state.mtx_nodes.try_lock())
            {
                ...
                for (auto& node : state.nodes)
                {
                    ...
                    ss << "[" << node->id << "] " << inet_ntoa(node->addr.sin_addr)
